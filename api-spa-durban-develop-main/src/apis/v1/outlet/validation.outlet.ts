...
export const create: { body: ObjectSchema } = {
  body: Joi.object().keys({
    ...
    email: Joi.string().custom(emailFormat),
    taxID: Joi.string().lowercase().required(),
    invoicePrefix: Joi.string()
      .custom(alphabetsOnly("invoicePrefix"))
      .required(),
    invoiceNumber: Joi.number().required(),

    onlinePaymentAccountId: Joi.string().custom(objectId),
  }),
}
...
export const update: { params: ObjectSchema; body: ObjectSchema } = {
  params: Joi.object().keys({
    outletId: Joi.required().custom(objectId),
  }),
  body: Joi.object().keys({
    name: Joi.string().lowercase().required(),
    address: Joi.string().lowercase().required(),
...
export const deleteDocument: { params: ObjectSchema } = {
  params: Joi.object().keys({
    outletId: Joi.string().custom(objectId),
  }),
}
