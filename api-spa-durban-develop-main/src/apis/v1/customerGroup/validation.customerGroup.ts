...
export const update: { params: ObjectSchema; body: ObjectSchema } = {
  params: Joi.object().keys({
    customerGroupId: Joi.required().custom(objectId),
  }),
  body: Joi.object().keys({
    customerGroupName: Joi.string().lowercase().required(),
    customers: Joi.array().items(Joi.string().custom(objectId).required()),
...
export const get: { query: ObjectSchema } = {
  query: Joi.object()
    .keys({
      ...
      customerGroupName: Joi.string().optional(),
      //   customers: Joi.objectid().optional(),
    })
    .optional(),
}
...
export const deleteDocument: { params: ObjectSchema } = {
  params: Joi.object().keys({
    customerGroupId: Joi.string().custom(objectId),
  }),
}
